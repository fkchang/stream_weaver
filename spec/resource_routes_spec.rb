...
RSpec.describe "ResourceDefinition routing (T2)" do
  ...
  describe "parse_path" do
    it "parses /posts as index" do
      expect(defn.parse_path('/posts')).to eq({ _sw_resource: :post, _sw_action: :index })
    end

    it "parses /posts/new as new" do
      expect(defn.parse_path('/posts/new')).to eq({ _sw_resource: :post, _sw_action: :new })
    end

    it "parses /post/:id as show" do
      expect(defn.parse_path('/post/42')).to eq({ _sw_resource: :post, _sw_action: :show, _sw_id: '42' })
    end

    it "parses /post/:id/edit as edit" do
      expect(defn.parse_path('/post/42/edit')).to eq({ _sw_resource: :post, _sw_action: :edit, _sw_id: '42' })
    end
    ...
    it "URL-decodes the id in show" do
      expect(defn.parse_path('/post/hello%20world')).to include(_sw_id: 'hello world')
    end

    it "URL-decodes the id in edit" do
      expect(defn.parse_path('/post/hello%20world/edit')).to include(_sw_id: 'hello world')
    end
    ...
    context "/posts/new ordering (collection before member)" do
      it "matches /posts/new as :new before /post/new-style-id as :show" do
        ...
        expect(new_state[:_sw_action]).to eq(:new)
        expect(new_state[:_sw_id]).to be_nil
      end
      ...
      it "does not match /post/new as :new (member singular path)" do
        ...
        expect(show_state[:_sw_action]).to eq(:show)
        expect(show_state[:_sw_id]).to eq('new')
      end
    end
  end
  ...
  describe "build_path" do
    it "builds /posts for index state" do
      st = { _sw_resource: :post, _sw_action: :index }
      expect(defn.build_path(st)).to eq('/posts')
    end

    it "builds /posts/new for new state" do
      st = { _sw_resource: :post, _sw_action: :new }
      expect(defn.build_path(st)).to eq('/posts/new')
    end

    it "builds /post/:id for show state" do
      st = { _sw_resource: :post, _sw_action: :show, _sw_id: '42' }
      expect(defn.build_path(st)).to eq('/post/42')
    end

    it "builds /post/:id/edit for edit state" do
      st = { _sw_resource: :post, _sw_action: :edit, _sw_id: '42' }
      expect(defn.build_path(st)).to eq('/post/42/edit')
    end

    it "returns nil when _sw_resource doesn't match" do
      st = { _sw_resource: :comment, _sw_action: :index }
      expect(defn.build_path(st)).to be_nil
    end

    it "returns nil for show/edit without id" do
      expect(defn.build_path({ _sw_resource: :post, _sw_action: :show })).to be_nil
      expect(defn.build_path({ _sw_resource: :post, _sw_action: :edit })).to be_nil
    end

    it "URL-encodes id in show path" do
      st = { _sw_resource: :post, _sw_action: :show, _sw_id: 'hello world' }
  ...
  describe "plural: override" do
    ...
    it "parses /people as index" do
      expect(person_defn.parse_path('/people')).to eq({ _sw_resource: :person, _sw_action: :index })
    end

    it "parses /people/new as new" do
      expect(person_defn.parse_path('/people/new')).to eq({ _sw_resource: :person, _sw_action: :new })
    end

    it "builds /people for index" do
      expect(person_defn.build_path({ _sw_resource: :person, _sw_action: :index })).to eq('/people')
    end

    it "builds /people/new for new" do
      expect(person_defn.build_path({ _sw_resource: :person, _sw_action: :new })).to eq('/people/new')
    end
  ...
  describe "state_for_path via App route chain" do
    ...
    it "resolves /posts to index state" do
      expect(app.state_for_path('/posts')).to eq({ _sw_resource: :post, _sw_action: :index })
    end

    it "resolves /posts/new to new state" do
      expect(app.state_for_path('/posts/new')).to eq({ _sw_resource: :post, _sw_action: :new })
    end

    it "resolves /post/5 to show state" do
      expect(app.state_for_path('/post/5')).to eq({ _sw_resource: :post, _sw_action: :show, _sw_id: '5' })
    end

    it "resolves /post/5/edit to edit state" do
      expect(app.state_for_path('/post/5/edit')).to eq({ _sw_resource: :post, _sw_action: :edit, _sw_id: '5' })
    end
  end
end
