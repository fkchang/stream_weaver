...
RSpec.describe 'University multi-course execution' do
  ...
  it 'runs a provider step from its course-qualified listener token without changing Getting Started' do
    ...
    token = 'btn_run_course_diagram_intent_run-course-diagram-intent-1'
    ...
    expect(StreamWeaver::University::Progress.load(course_id: 'diagram-intent').requested_at(1)).to be_a(String)
  ...
  it 'marks and resets only the course identified by the control token' do
    ...
    StreamWeaver::University::Listener.handle_event(
      { data: { button: 'btn_mark_done_course_diagram_intent_mark-done-course-diagram-intent-1' } }
    )

    expect(StreamWeaver::University::Progress.load(course_id: 'diagram-intent').done_steps).to eq([1])

    StreamWeaver::University::Listener.handle_event(
      { data: { button: 'btn_reset_course_diagram_intent_reset-course-diagram-intent' } }
    )
  ...
  it 'resolves the selected course demo while omitted selection stays on Getting Started' do
    expect(StreamWeaver::University::Demos.path('diagram', course_id: 'diagram-intent')).to eq(@provider_demo)
