...
7. **Run → Re-run.** `canvas.rb`'s per-row primary button (`id: "run-#{number}"`, the `:current`/`:todo` branches — `:done`'s "Repeat" is untouched) reads "Re-run" once `progress.requested_at(number)` is set (permanent, stamped only on a `:sent` status) OR the step's own `last_run` is the most recent click (covers a degraded/failed send, which never earns a `requested_at` stamp). Scoped deliberately to the row button only — the hero "Run step N" button (`canvas.rb` ~line 683) does NOT pick up the same relabeling; a real inconsistency (hero says "Run step 3", the row a few lines down says "Re-run" for the same step) that a future pass should either fix by applying the same rule there, or resolve by making the row-vs-hero distinction explicit in a comment. Flagged in round-6 code review, not fixed — out of the literal ask's scope ("a step row's primary button").

Known follow-ups from round-6 code review, not done (small, non-blocking): hero button Re-run consistency (above); `run_label` could move into `module Canvas` alongside `step_states`/`mark_done_message` for unit-testability without a full HTML render; `Progress` could expose a `last_run_on?(step_number)` predicate instead of three call sites cracking open the raw `last_run` hash directly.
