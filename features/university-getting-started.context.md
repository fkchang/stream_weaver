...
## Round-11: the stale-bridge trap heals itself (disc-171, 2026-09-04)

**The trap.** The canvas bridge is a long-lived process holding its classes in memory. A `rake install` or `gem update` underneath it leaves it serving code the caller no longer has, so freshly pushed canvas source calling a brand-new method dies with a bare `NameError` (seen live on `Progress#last_done`). Hit three times, including during a retrospective demo. The manual cure -- `streamweaver canvas-restart` -- only helps someone who already knows the trap exists, which is exactly the clean-room walkthrough's blind spot.

**The stamp.** `Canvas::CodeStamp` (`lib/stream_weaver/canvas/code_stamp.rb`, new) fingerprints the code a process loaded: `version=<StreamWeaver::VERSION>` plus `code=<path of the loaded stream_weaver.rb>:<mtime epoch>`. `BridgeServer.write_pid_file` appends both lines to `~/.streamweaver/canvas.pid` at boot. The existing `pid=`/`port=` readers are regex-based, so a pre-stamp two-line pid file still parses -- and a missing stamp counts as stale, which is precisely the upgrade case. Version alone would not catch anything: the trap fires hardest on a *same-version* reinstall, hence the mtime.

**The chokepoint.** `Canvas::StalenessGuard.ensure_current!` (`lib/stream_weaver/canvas/staleness_guard.rb`, new) runs at the top of `Client#send_message`, `send_and_wait`, `each_event` and `ensure_bridge_running` -- the four methods every bridge-facing command funnels through, rather than N call sites (`canvas-push` in particular never calls `ensure_bridge_running`, so guarding that alone would have missed it). The policy lives in its own module rather than in `Client`, which is otherwise about sockets and `Protocol`. It checks **once per process**, which is also what makes recursion impossible: the heal's own snapshot/restore traffic re-enters the guard and finds the check already spent. `canvas-stop` and `canvas-restart` call `StalenessGuard.disable!` first -- they are the manual spelling of the same cure.

**The heal.** One line on **stderr** (`StreamWeaver: canvas bridge was running older code — restarting it (sessions preserved)…`), then `CLI.restart_bridge_preserving_sessions` -- the programmatic core of `canvas-restart` (snapshot -> stop -> start -> `wait_for_bridge_ready` -> restore) with none of the command's narration, its UNCONFIRMED prompt (nobody is there to answer it) or its `exit 1`. stderr, not stdout, because `canvas-wait` exists to emit nothing but JSON on stdout for a script to parse -- narration in front of it is a parse error on the other end. The restart's own output is swallowed so the heal costs the caller that one line, but four things still speak on stderr: sessions the snapshot could not capture, a **port move** (it returns `{ ok:, dir:, unconfirmed:, old_port:, port: }` -- an auto-restart that silently moves the bridge leaves every open tab dead on the old port, which is disc-171's own symptom reproduced by the cure; `canvas-restart` warns about this in stars, so the automatic path owes at least as much), a restore that failed, and a restart that RAISED -- rescued, because a cure that cannot finish must not take down the command that only wanted to push a canvas.

**False positives are the real risk**, not false negatives: a spurious restart takes down every session the developer has open. Identical version+path+mtime never fires; an unknowable local fingerprint (no library file to stat) never fires. And a **60s cooldown** (`~/.streamweaver/canvas.heal`, written *before* the restart so a second process arriving mid-heal backs off) means the guard never restarts twice in a row: two installs cannot land inside a minute, so a second mismatch that soon is two callers disagreeing about which code is current -- a dev checkout and the installed gem, reachable from this repo's own daily workflow -- and healing again would hand the bridge back and forth forever. That branch points at `SW_NO_AUTO_RESTART=1`, not at `canvas-restart` -- the latter is advice to resume the fight. `spec_helper` now also sets `SW_NO_AUTO_RESTART=1` suite-wide, same reasoning as the existing `SW_NO_OPEN=1` line -- any spec that stubs the bridge as running can reach the guard, and without a redirected pid file the bridge it would restart is the developer's live one. The two specs that exercise the heal clear it locally alongside their own throwaway socket/pid.

**Known gap:** editing one file deep in a dev checkout is not detected (the fingerprint tracks `stream_weaver.rb`, which an install rewrites and a targeted edit does not). That case still wants an explicit `canvas-restart`. Documented in `docs/canvas-panel-workflow.md` under "Upgrading StreamWeaver under a running bridge".

E2E evidence: `spec/canvas/bridge_staleness_e2e_spec.rb` boots a real bridge on a throwaway socket, pushes two sessions, doctors the pid file's `code=` line to look post-install, then runs a real `CLI.canvas_push` -- asserting one heal line on stderr, `Pushed to ...` alone on stdout, a different bridge pid, the untouched session still serving its original content, and the push landed on the new bridge. Companion cases prove a matching bridge is left completely alone, that `SW_NO_AUTO_RESTART` warns without restarting, and that a second stale reading right after a heal warns instead of ping-ponging the bridge.

Full suite: `bundle exec rspec` -> 4148 examples, 0 failures, 1 pending (pre-existing browser-only).

## The artifact lifecycle: manifest, cleanup, stop (2026-09-04)

The course used to be a one-way door: it saved docs, exported `.org` files, published gists and opened canvas sessions, and nothing knew what it had left behind. Three pieces, one slice.

**The manifest is an allowlist, not an inventory.** `University::Artifacts` (`lib/stream_weaver/university/artifacts.rb`, `~/.streamweaver/university/artifacts.yml`, `STREAMWEAVER_UNIVERSITY_ARTIFACTS`) records `{type, ref, step, created_at}` for four types — `doc`, `org`, `gist`, `session`. It exists for exactly one consumer, `University::Cleanup`, which deletes a thing only if the manifest currently records it. That inverts the usual reading: a ref that gets *in* is a ref cleanup is later allowed to *destroy*, so `record!` refuses anything whose type it cannot determine rather than guessing, and `record_session!` is guarded on `Listener::DEMO_SESSION_NAMES` — the same allowlist `university-reset` closes by, resolved lazily (`Artifacts.demo_session_names` requires listener inside the method) because listener requires artifacts at load time and a top-level pair would be a cycle.

Recording is invisible where it can be: `growing_doc.rb` records its own session on create and its saved path after `save_doc` (plus an `.org` sibling if one already exists), `Listener.warm_up!` records the demo session it creates. A gist can't be: it exists only in the worker's own `gh gist create` output, so step 5's prompt has the worker run `streamweaver university-artifact add <url> --step 5` (and the same for the `.org`). That is the whole reason the CLI door exists.

**One deletion implementation, two surfaces.** `University::Cleanup` (`lib/stream_weaver/university/cleanup.rb`) is the only code that deletes anything, and it never prompts — confirmation belongs to the surface, because the two surfaces confirm in completely different ways and only one of them has a tty. Every guard is re-checked against freshly read state at delete time, not against what the caller was holding:

- `delete_entry!` raises `Cleanup::Refused` for a ref not currently in the manifest, and again for a `session` ref not in `DEMO_SESSION_NAMES` (a hand-edited manifest must not be able to name the controller canvas).
- `delete_state_file!` refuses any path not in `state_files`, which is an allowlist by exact basename (`progress.yml`, `progress.yml.bak`, `worker.json`, `listener.pid`, `listener.log`, `artifacts.yml`, plus `<demo>_state.yml`) under `dirname(Progress.path)` — never a `Dir[dir/*]` sweep, because that directory is in the user's home.
- A gist is deleted only via `gh gist delete <id> --yes`, only for a ref that parses as a gist URL, and a failure (or a missing `gh`) leaves the manifest entry alone so the URL can still be found.
- A file or session already gone is `ok` — the end state the caller asked for is the end state it got.

CLI: `streamweaver university-cleanup` prints the grouped inventory (sizes, missing files flagged), then confirms per GROUP — except gists, one at a time showing the URL, since a gist is the only artifact that left the machine. `--dry-run` prints and prompts for nothing. Canvas: the completion recap gained an Artifacts section whose buttons only ever *ask* (`cleanup-ask-docs|orgs|sessions|gist-N`) — they write a pending confirmation and the listener's own re-push renders "Really delete X? [Confirm delete] [Keep]", so nothing on the canvas can delete in one click. `Listener.cleanup_ask!` resolves the target to REFS at ask time, not at confirm time: what the user is shown is exactly what goes, and an index cannot drift underneath a confirmation. The pending record and the last delete's report live in artifacts.yml (not `Progress`) for the same reason `viewing` lives on disk at all — canvas.rb is instance_eval'd fresh on every push, so nothing about what is on screen survives in memory.

**`streamweaver university-stop` is neither reset nor cleanup.** It stops the listener (`Listener.stop!`, which already checks the pid is alive AND a process-group leader), closes the demo canvas sessions, and closes the two iTerm2 sessions `get-started` recorded in worker.json (`session_id`, `controller_session_id`) via `ITerm.close_pane`, skipping silently for one that is dead or was never recorded. It keeps progress.yml and artifacts.yml. `close_demo_sessions!` gained `clear_state:` for this: reset means start over (clear growing_doc's picks), stop means put it down (keep them, so the doc the user returns to is the doc they left).

**Review round (DHH, same day) found three real holes, all in the same shape — two doors disagreeing about the same question:**

1. `infer_type` admitted any URL containing `gist.github.com`; `gist_id` demanded a hex id. A URL loose enough to record could be too vague to delete — a permanently stuck manifest entry that raised on every attempt. `Artifacts.gist_id` is now the single definition, `infer_type` calls it, `record!` refuses a gist ref it produces nothing for, and Cleanup calls it again as its second guard.
2. `Cleanup::Refused` escaped the CLI (the canvas rescued it, the CLI did not) — a backtrace mid-run, with some files already deleted and the remaining groups never offered. `Cleanup.delete_refs!` is now the batch entry point BOTH surfaces use, reporting a refusal as a not-ok outcome so the rest of the batch still gets its turn; `delete_entry!` still raises, because a single delete has nowhere to put a report.
3. `university-artifact add --type doc /tmp/a.rb` recorded the ref `"doc"` — the positional scan took the flag's value. A relative ref is then resolved against whatever process runs cleanup. Fixed at both ends: `CLI.positional` skips valued flags, and `record!` stores doc/org refs `File.expand_path`ed so the manifest is cwd-independent by construction. Flags-before-positional is exactly the shape an agent produces, and step 5's prompt hands this command to one.

Also from that round: `inventory` is keyed by `Artifacts::TYPES` (plus `Cleanup::STATE`) rather than symbols of its own, so pairing the wrong group with the wrong type is unrepresentable; labels live once in `Artifacts::LABELS`; `state_files` derives the manifest and growing-doc sidecar names from the modules that write them and then filters back to `state_dir`, so a partially-overridden environment cannot reach out of the directory it was scoped to; `Artifacts.write` takes an flock, because the listener records while a foreground cleanup forgets. Deliberately NOT done: unifying the four `STREAMWEAVER_UNIVERSITY_*` env vars behind one `University.state_dir` (right, but a refactor of its own), and a containment guard on doc/org paths — instead `Cleanup`'s header now says plainly that doc/org have no second guard and that `Artifacts` is the strict door for them, since safety code that over-promises is worse than safety code that doesn't.
