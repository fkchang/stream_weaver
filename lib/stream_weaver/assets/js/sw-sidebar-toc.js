...
(function() {
  ...
  function initSidebarToc() {
    ...
    // A real htmx swap replaces navEl outright, so this flag is naturally
    // fresh there. A host that re-invokes init without a real swap (the
    // browser extension's sandbox, which has no htmx at all) would
    // otherwise attach a second click listener to every link and leak a
    // second IntersectionObserver watching the first one's now-detached
    // targets -- not hypothetical, this exact host now calls init directly.
    if (!navEl || navEl.dataset.swTocReady) return;
    navEl.dataset.swTocReady = 'true';
