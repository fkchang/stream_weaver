...
(function () {
  ...
  function cloneSvgWrapper(svg) {
    ...
    //
    // Set via setAttribute, not svgEl.style.*: a first version used
    // .style, which worked everywhere it was tested locally but not on
    // SharePoint -- some corporate CSPs restrict style-src-attr (inline
    // style="", whether written in markup or set via the .style DOM API)
    // independently of style-src, which governs the <style> block
    // elsewhere on this page and was never blocked. width=/height= are
    // plain SVG geometry attributes, not CSS, so no style-src directive
    // governs them at all -- see the matching fix and longer comment on
    // the expand button's own icon in adapter/alpinejs.rb. removeAttribute
    // avoids the same class of risk for clearing mermaid's own inline
    // max-width, which is the only thing that attribute contains.
