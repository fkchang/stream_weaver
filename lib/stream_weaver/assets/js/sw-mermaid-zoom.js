...
(function () {
  ...
  function cloneSvgWrapper(svg) {
    ...
    // Mermaid emits an inline style="max-width: Npx" on the root <svg> --
    // an inline style beats any non-!important stylesheet rule, so the
    // CSS max-width:none on this class alone would be silently overridden
    // by mermaid's own attribute. Clearing it here is what actually lets
    // the diagram render at natural size instead of the width it was
    // originally rendered at.
    wrapper.firstElementChild.style.maxWidth = 'none';
    return wrapper;
  }
