...
(function () {
  ...
  function getThemeVariables() {
    ...
    return {
      ...
      lineColor: toRgbColor(style.getPropertyValue('--sw-border').trim()) || (isDark ? 'rgba(255,255,255,0.1)' : '#e0e0e0'),
  ...
  function renderDiagram(container, mermaid, customVars) {
    ...
    // swMermaidInit() can be called concurrently (Alpine x-init + explicit polling call),
    // and mermaid.render() is async — without this guard, two renders race on the same
    // container ID, which corrupts mermaid's internal state.
