...
(function () {
  ...
  // Which containers and expand buttons are already wired, by identity.
  //
  // Node identity, not a data attribute. Two things re-run renderDiagram
  // against a container that already exists, and both of them clear
  // data-sw-mermaid-done first: reRenderAll() (a theme toggle) clears it
  // explicitly, and a morphdom patch (the browser extension's sandbox, via
  // SWRuntime) clears it by syncing attributes from freshly rendered markup,
  // which is also the moment it strips the rendered SVG -- so in both cases a
  // fresh render is genuinely wanted. What must NOT be redone either time is
  // the listener wiring, and a morphdom patch would strip an attribute guard
  // for it just as readily while keeping the node the listeners sit on.
  //
  // (OpalRuntime#morph_options now asks morphdom to skip a container that
  // still carries data-sw-mermaid-done, so a finished diagram is usually left
  // alone entirely. A container caught mid-render is not, which is exactly the
  // case these guards have to keep covering.)
  var zoomWired = new WeakSet();
  var expandWired = new WeakSet();

  function initZoomPan(container, diagramEl) {
    // Without this guard, every re-render stacks another full set of
    // wheel/mousedown/document-mousemove/document-mouseup listeners (the last
    // two unremovable, with no AbortController) plus another click handler per
    // button, each closing over its own independent zoom state.
    if (zoomWired.has(container)) return;
    zoomWired.add(container);
  ...
  function initExpand(container, diagramEl) {
    ...
    // A re-render replaces diagramEl's innerHTML but never recreates
    // .sw-mermaid__controls, so without this guard every one stacks another
    // click handler on the same button, and each stacked handler opens (and
    // immediately re-closes) its own overlay + AbortController on a single
    // click. Keyed on the button node for the reason zoomWired is -- see there.
    if (!btn || expandWired.has(btn)) return;
    expandWired.add(btn);
